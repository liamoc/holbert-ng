      ...
      | Some((subterm, localScope)) =>
        ...
        | Some((eqLhs, eqRhs)) =>
          ...
          if !Term.equivalent(lhs', subterm) {
            ...
            let newGoal = Term.replaceAt(goal, step.path, Term.reduce(Term.upshift(rhs, localScope->Array.length)))
...
let apply = (
  ...
  let actionsFor = (ruleName, rule: Rule.t): array<(t<'a>, Term.subst)> =>
    switch Term.asEquation(rule.conclusion) {
    | None => []
    | Some(_) => {
        ...
        | Some((eqLhs, eqRhs)) =>
          [Forward, Backward]->Array.flatMap(direction => {
            ...
            Term.positions(goal)->Array.filterMap(((path, subterm, boundScope)) =>
              ...
              | Some(subst) =>
                ...
                let newGoal = Term.replaceAt(goal, path, replacement)
    ...
    | many => [
        Results.Group(
          ...
          many->Array.map(((step, subst)) => 
            Results.Action(`${SHOTerm.prettyPrintPath(step.path)} ${dirLabel(step.direction)}`, step, subst)),
        ),
      ]
    }
  })
...
let prettyPrint = (
    ...
    ->String.concat(") ")
    ->String.concat(SHOTerm.prettyPrintPath(it.path))
    ->String.concat(" {")
    ->String.concat(
  ...
  if cur.contents->String.get(0) == Some("(") {
    ...
    | Ok((ruleName, rest)) => {
        ...
        if cur.contents->String.get(0) == Some(")") {
          ...
          let (path,rest) = SHOTerm.parsePath(cur.contents);
          cur := String.trim(rest);
          let subgoals = []
          if cur.contents->String.get(0) == Some("{") {
            cur := String.trim(cur.contents->String.sliceToEnd(~start=1))
